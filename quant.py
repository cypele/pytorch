...
class CifarResNet(nn.Module):
    def __init__(self,):
        super().__init__()
        efficientnet = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1)
        self.last_feature_layer = 'flatten'
        return_nodes = [self.last_feature_layer]
        self.backbone = create_feature_extractor(efficientnet, return_nodes)
 
        self.backbone.eval()
        with torch.no_grad():
            out_data = self.backbone(torch.rand((1, 3, 32, 32)))[self.last_feature_layer]
        classifier_input_shape = out_data.shape[1]
        self.classifier = nn.Sequential(nn.Dropout(p=0.2, inplace=True), nn.Linear(in_features=classifier_input_shape, out_features=10))
    def forward(self, x):
        x = self.backbone(x)[self.last_feature_layer]
        x = self.classifier(x)
        return x
...
def load_data(train=True,
  ...
  # train_subset, val_subset = random_split(trainset, [0.8, 0.2])
  # test_loader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=2)

  if train:
    # dataset = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=2)
    # if subset_len:
    ...
    # dataset = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=2)
    # if subset_len:
    #   assert subset_len <= len(dataset)
    #   if sample_method == 'random':
    #     dataset = torch.utils.data.Subset(
    ...
    data_loader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=2)
